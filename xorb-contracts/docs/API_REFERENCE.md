# Xorb Smart Contracts - Complete API Reference
...
Mints new XORB tokens to the specified address.
...
Returns contract's XORB token balance.
...
#### `wrap(uint256 amount)`
Wraps XORB tokens to create USDC.

**Parameters:**
- `amount` (uint256): Amount of XORB to wrap

**Behavior:**
- Transfers XORB from user
- Mints equivalent USDC
- Updates staking start time if first wrap

#### `unwrap(uint256 amount)`
Unwraps USDC back to XORB.

**Parameters:**
- `amount` (uint256): Amount of USDC to unwrap
...
This API reference covers all public and external functions in the Xorb smart contract ecosystem. For implementation details and internal functions, refer to the source code comments.