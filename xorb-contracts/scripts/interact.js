 ...
 * Provides easy-to-use functions for interacting with deployed Xorb contracts
 */
class XorbContracts {
  constructor() {
    this.contracts = {};
   ...
   */
  async initialize() {
    console.log("🔗 Initializing Xorb Contracts...");

    const addresses = {
      XorbToken: process.env.XORB_TOKEN_ADDRESS,
      WrappedXORB: process.env.WRAPPED_XORB_ADDRESS,
      PoPTracker: process.env.POP_TRACKER_ADDRESS,
      MetaTransactionPaymaster: process.env.META_TRANSACTION_PAYMASTER_ADDRESS,
      XorbGovernance: process.env.XORB_GOVERNANCE_ADDRESS
    };
  ...
  /**
   * Get XORB token balance for an address
   */
  async getXORBBalance(address) {
    if (!this.contracts.XorbToken) {
      throw new Error("XorbToken not loaded");
    }
    const balance = await this.contracts.XorbToken.balanceOf(address);
   ...
   */
  async getWXORBBalance(address) {
    if (!this.contracts.WrappedXORB) {
      throw new Error("WrappedXORB not loaded");
    }
    const balance = await this.contracts.WrappedXORB.balanceOf(address);
    return ethers.formatEther(balance);
  }
  ...
  async getPendingRewards(address) {
    ...
    const rewards = await this.contracts.WrappedXORB.getPendingRewards(address);
  ...
  async wrapTokens(amount, signer) {
    ...
    const tx = await this.contracts.WrappedXORB.connect(signer).wrap(amountWei);
    await tx.wait();
    console.log(`✅ Wrapped ${amount} XORB tokens`);
    return tx.hash;
  }
  ...
  async unwrapTokens(amount, signer) {
    ...
    const amountWei = ethers.parseEther(amount.toString());
    const tx = await this.contracts.WrappedXORB.connect(signer).unwrap(amountWei);
    await tx.wait();
    console.log(`✅ Unwrapped ${amount} USDC tokens`);
    return tx.hash;
  }
  ...
  async claimRewards(signer) {
    ...
    const tx = await this.contracts.WrappedXORB.connect(signer).claimRewards();
    await tx.wait();
  ...
  async createProposal(targets, values, calldatas, title, description, evidence, signer) {
    if (!this.contracts.XorbGovernance) {
      throw new Error("XorbGovernance not loaded");
    }
    
    // Convert values to wei
    const valuesWei = values.map(v => ethers.parseEther(v.toString()));
    
    const tx = await this.contracts.XorbGovernance.connect(signer).propose(
      targets,
      valuesWei,
      calldatas,
      title,
  ...
  async voteOnProposal(proposalId, support, reason, signer) {
    ...
    const tx = await this.contracts.XorbGovernance.connect(signer).vote(
      proposalId,
      support,
      reason
    );
  ...
  async lockGovernanceFunds(amount, signer) {
    ...
    const amountWei = ethers.parseEther(amount.toString());
    const tx = await this.contracts.XorbGovernance.connect(signer).lockFunds(amountWei);
    await tx.wait();
    console.log(`✅ Locked ${amount} XORB for governance`);
    return tx.hash;
  }
   ...
   * Transfer tokens
   */
  async transferTokens(to, amount, signer, tokenName = "XorbToken") {
...
async function runDemo() {
  console.log("🎬 Xorb Contract Interaction Demo\n");
  
  const xorbContracts = new XorbContracts();
  await xorbContracts.initialize();
  
  // Get signers
  const [deployer, user1] = await ethers.getSigners();
  
  console.log("📊 Contract Information:");
  const contractInfo = await xorbContracts.getContractInfo();
  console.log(JSON.stringify(contractInfo, null, 2));
  
  console.log("\n💰 User Balances:");
  const xorbBalance = await xorbContracts.getXORBBalance(user1.address);
  const USDCBalance = await xorbContracts.getWXORBBalance(user1.address);
  const pendingRewards = await xorbContracts.getPendingRewards(user1.address);
  
  console.log(`User1 XORB Balance: ${xorbBalance}`);
  console.log(`User1 USDC Balance: ${USDCBalance}`);
...
async function main() {
  ...
  if (command === "demo") {
    ...
    const xorbBalance = await xorbContracts.getXORBBalance(address);
    const USDCBalance = await xorbContracts.getWXORBBalance(address);
    const pendingRewards = await xorbContracts.getPendingRewards(address);
    
    console.log(`Address: ${address}`);
    console.log(`XORB Balance: ${xorbBalance}`);
    console.log(`USDC Balance: ${USDCBalance}`);
    console.log(`Pending Rewards: ${pendingRewards}`);
  } else {
    console.log(`
Xorb Contract Interaction CLI
...
Environment variables required:
  XORB_TOKEN_ADDRESS
  WRAPPED_XORB_ADDRESS
  POP_TRACKER_ADDRESS
  META_TRANSACTION_PAYMASTER_ADDRESS
  XORB_GOVERNANCE_ADDRESS
    `);
  }
}
