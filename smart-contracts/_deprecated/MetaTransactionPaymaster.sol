...
contract MetaTransactionPaymaster is EIP712, AccessControl, ReentrancyGuard {
    ...
    function executeMetaTransaction(
        ...
        userData.nonce = userData.nonce.add(1);
    ...
    function _handlePayment(
        ...
        if (token == address(0)) {
            ...
            if (msg.value > paymentAmount) {
                payable(user).call{value: msg.value - paymentAmount}("");
            }
        } else {
            // ERC20 token payment
        ...
        // Collect fee
        uint256 fee = payment.mul(feePercentage).div(10000);
        totalFeesCollected = totalFeesCollected.add(fee);
        tokenFeesCollected[token] = tokenFeesCollected[token].add(fee);
    ...
    function _checkRateLimits(address user) internal view {
        ...
        require(block.timestamp >= userData.lastTransaction.add(rateLimits.cooldownPeriod), "Cooldown period active");
    }
    ...
    function _updateRateLimits(address user, uint256 gasUsed) internal {
        ...
        userData.rateLimitCount = userData.rateLimitCount.add(1);
        userData.usedQuota = userData.usedQuota.add(gasUsed);
        userData.transactionCount = userData.transactionCount.add(1);
    }
