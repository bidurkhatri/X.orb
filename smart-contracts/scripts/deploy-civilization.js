...
/**
 * Deploys the SylOS Agent Civilization contracts:
 ...
 *   - WrappedSYLOS (wSYLOS) — used as stake token
 *
 * Cross-contract permissions are configured automatically.
 */
async function main() {
  console.log("=".repeat(70));
  console.log("  SylOS Agent Civilization — Contract Deployment");
  console.log("=".repeat(70));
  ...
  // If deploying alongside existing contracts, set the wSYLOS address:
  const WSYLOS_ADDRESS = process.env.WSYLOS_ADDRESS || "";
  const TREASURY_ADDRESS = process.env.TREASURY_ADDRESS || deployer.address;
  const ADMIN_ADDRESS = process.env.ADMIN_ADDRESS || deployer.address;

  let wrappedSylosAddress = WSYLOS_ADDRESS;

  // If no existing wSYLOS, deploy full stack (for testing)
  if (!wrappedSylosAddress) {
    console.log("No WSYLOS_ADDRESS set — deploying full token stack...\n");

    // Deploy SylOSToken
    console.log("[1/8] Deploying SylOSToken...");
    const SylOSToken = await ethers.getContractFactory("SylOSToken");
    const sylToken = await SylOSToken.deploy(
      "SylOS Token",
      "SYLOS",
    ...
    console.log(`  SylOSToken: ${sylTokenAddr}`);

    // Deploy WrappedSYLOS
    console.log("[2/8] Deploying WrappedSYLOS...");
    ...
    console.log(`  WrappedSYLOS: ${wrappedSylosAddress}`);
  } else {
    console.log(`Using existing wSYLOS at: ${wrappedSylosAddress}\n`);
  }

  // ---------------------------------------------------------------
  // Deploy Agent Civilization Contracts
  // ---------------------------------------------------------------
  ...
  const contracts = {
    WrappedSYLOS: wrappedSylosAddress,
