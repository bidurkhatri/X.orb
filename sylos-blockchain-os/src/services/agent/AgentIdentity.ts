 ...
 * @description Agent Agent Identity — Full Profile for Licensed Workers
 *
 * Every agent in Xorb is a full agent with:
...
const IDENTITY_KEY = 'xorb_agent_identities'
...
class AgentIdentityService {
    ...
    private loadFromStorage(): void {
        try {
            ...
            const profiles: AgentProfile[] = parsed.profiles || []
            profiles.forEach(p => this.profiles.set(p.agentId, p))
            this._loaded = true
        } catch (e) {
            console.error('[AgentIdentity] Failed to load:', e)
            this._loaded = true
        }
    }
    ...
    private saveToStorage(): void {
        try {
            ...
            console.error('[AgentIdentity] Failed to save:', e)
        }
    }
    ...
    /**
     * Create a full agent profile for a newly spawned agent.
    ...
    createProfile(params: {
        ...
        const profile: AgentProfile = {
            ...
            actionHistory: [{
                ...
                type: 'STATUS_CHANGE',
                description: `Agent ${params.name} born into Xorb network as ${params.role}`,
                timestamp: now,
        ...
        this.saveToStorage()
        console.log(`[AgentIdentity] Agent profile created for ${params.name} (${designation})`)
        return profile
    }

    /* ─── Profile Updates ─── */

    /** Record an action in the agent's life history */
    ...
    /** Update agent status */
    updateStatus(agentId: string, status: AgentProfile['status']): void {
        const profile = this.profiles.get(agentId)
        if (!profile) return
    ...
    private updateLifestyle(profile: AgentProfile, action: Omit<ActionRecord, 'id'>): void {
        // Track peak hours
    ...
    getProfileSummary(agentId: string): AgentProfileSummary | undefined {
        const p = this.profiles.get(agentId)
        if (!p) return undefined

        return {
            agentId: p.agentId,
            name: p.birth.networkName,
...
export const agentIdentity = new AgentIdentityService()
